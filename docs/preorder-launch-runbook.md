...
- Apply `20260810120000_track_preorder_email_delivery_outcomes.sql` before deploying its matching source. Keep the signature-verified Resend endpoint subscribed to `email.sent`, `email.delivered`, `email.delivery_delayed`, `email.failed`, `email.bounced`, `email.complained` and `email.suppressed`; new pre-order sends must record a delivered outcome rather than only API acceptance.
...
- Run `npm run preorder:check:operations:test` after webhook, email, cancellation, refund, delivery-deadline, fulfilment or inventory changes. The live form is `npm run preorder:check:operations`. It is read-only and returns `SAFE TO ACCEPT ORDERS` only when there are no failed or stalled operations, delayed/bounced/complained/suppressed or ten-minute-unconfirmed email outcomes, overdue delivery actions, unresolved order workflows, or paid/reserved inventory inconsistencies.
...
9. Set the paused live released-unit ceiling to exactly one, run `npm run preorder:check:email`, `npm run preorder:check:payments`, `npm run preorder:check:operations`, then `npm run preorder:check:live-smoke`; do not continue unless all report zero failures. The checks must confirm email identity and authentication, exact payment/order reconciliation, healthy operations and inventory, plus `charges_enabled`, `payouts_enabled`, active card capability, no current/past/pending verification requirements, the incorporated legal identity, customer-support profile, statement descriptor, automatic payout schedule, agreement acceptance, and Stripe branding.
...
11. Verify the charged subtotal, shipping and tax, signed webhook, live order record, delivered provider outcomes for the confirmation and operations emails, and secure management link. Issue a full refund and wait for the reconciled `refunded` state.
...
13. Run `npm run preorder:check:payments`, `npm run preorder:check:operations`, then `npm run preorder:check:launch`; they must reconcile and verify the exact private live order with zero failures and explicitly report that operations are safe. Confirm there are no failed webhooks, failed emails, pending cancellations, payment mismatches, overdue delivery actions, or inventory discrepancies.
